...
This design closes two common evaluation bugs. First, dictionary lookup with missing keys can make $m(\bot,\bot)$ appear true. Second, dropping failed runs conditions the analysis on success and can inflate both accuracy and agreement. \pantheon retains all prespecified tasks and scores nulls as incorrect and non-agreeing.
...
We use the 569-row, 30-feature Breast Cancer Wisconsin Diagnostic dataset bundled with scikit-learn. Ten stratified 75/25 splits are fixed in advance; scaling is fit on training data only. The proposer is scikit-learn L-BFGS logistic regression. The replicator is an independent numpy batch-gradient-descent implementation. The measured effect is improvement in test accuracy over the test-set majority baseline. Because the ten test splits overlap, split summaries are descriptive and are not treated as independent replications.
...
\centering
\scriptsize
\input{../results/external/table_external_rigorous.tex}
...
The principal societal risk is false assurance: a green audit can be mistaken for scientific truth. We therefore separate structural gate completion from scientific readiness. The repository's legacy gate string \texttt{PUBLICATION-READY} means that the internally frozen artifact matrix existed and its structural validators passed; it does not override the 0/20 capability result or imply venue readiness.

\section{Reproducibility}
The definitive protocol identifier is \texttt{PANTHEON-COREBENCH-OOD-LOCAL-20260903-V4}. Its JSON file records all source-file hashes, the complete manifest hash, model digests, platform, decoding limits, isolation policy, failure semantics, and claim boundary. The archive manifest records a SHA-256 for every source capsule and canonical answer file. At the V4 freeze, twenty-two tests covered hashing, package execution, statistics, external scoring, provider normalization, failure semantics, and safe extraction; later workflow-integrity tests are post-outcome maintenance rather than evidence fixed before execution. Controlled evidence validation checks all 480 classifier records, 200 executions, 20 public-case runs, and required figures.

The preserved local evidence is revalidated with \texttt{bash scripts/reproduce\_all.sh}, and the rigorous post-run analysis is \texttt{python3 analysis/analyze\_external\_rigor.py}. V4 was internally frozen before execution but was not externally timestamped or registered. Its external matrix was produced historically with \texttt{bash scripts/run\_local\_ood\_study.sh}; the repaired current source intentionally stops that command before execution because it no longer matches the frozen V4 source manifest. A fresh confirmation requires an externally timestamped, separately frozen V5 protocol, model identities, and versioned output namespaces. All reported values are read from persisted CSV or JSON artifacts; no result in this paper is manually substituted.
...
Before using \pantheon\ evidence to support a scientific claim, a reader should verify: (1) all prespecified tasks are retained; (2) non-null completion is high enough for agreement analysis; (3) canonical answers and tolerances were fixed before execution; (4) canonical data were inaccessible to agents; (5) model and source identities are exact; (6) shared provider, prompt, and environment dependencies are disclosed; (7) canonical divergence is interpreted by a domain expert; and (8) a structural gate is not reported as scientific correctness.
