...
def test_complete_artifact_can_fail_capability_floor():
    ...
    tasks = pd.DataFrame(
        {"agent_a_submitted_report": [False] * 17, "agent_b_submitted_report": [False] * 17}
    )
...
def test_capability_floor_requires_joint_answers_and_authored_reports():
    ...
    tasks = pd.DataFrame(
        {"agent_a_submitted_report": [True] * 5, "agent_b_submitted_report": [True] * 5}
    )
    result = module.assess(questions, tasks, {"verdict": "PUBLICATION-READY"})
...
def test_explicit_incomplete_structure_cannot_be_overridden_by_legacy_label():
    module = _module()
    questions = pd.DataFrame({"agent_a_answered": [1] * 5, "agent_b_answered": [1] * 5})
    tasks = pd.DataFrame(
        {"agent_a_submitted_report": [True] * 5, "agent_b_submitted_report": [True] * 5}
    ...
    result = module.assess(
        questions,
        tasks,
        {"artifact_complete": False, "verdict": "PUBLICATION-READY"},
    )
