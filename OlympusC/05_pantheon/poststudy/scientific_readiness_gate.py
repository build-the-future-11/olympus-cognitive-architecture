...
def assess(
    ...
    *,
    ...
    if not required_q.issubset(question_rows.columns) or not required_t.issubset(task_rows.columns):
        missing = sorted((required_q - set(question_rows.columns)) | (required_t - set(task_rows.columns)))
    ...
    joint_answered = int((question_rows.agent_a_answered.astype(bool) & question_rows.agent_b_answered.astype(bool)).sum())
    ...
    artifact_complete = legacy_gate.get("verdict") == "PUBLICATION-READY"
    capability_checks = {
    ...
    return {
        ...
        "gate_timing": "Post-study reporting safeguard; thresholds are not presented as preregistered hypothesis tests.",
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--questions", type=Path, default=Path("results/external/question_level.csv"))
    parser.add_argument("--tasks", type=Path, default=Path("results/external/external_task_results.csv"))
    parser.add_argument("--legacy-gate", type=Path, default=Path("PUBLICATION_READINESS.json"))
    parser.add_argument("--output", type=Path, default=Path("poststudy/SCIENTIFIC_READINESS.json"))
    args = parser.parse_args()
    result = assess(pd.read_csv(args.questions), pd.read_csv(args.tasks), json.loads(args.legacy_gate.read_text()))
    ...
    md_path.write_text(
        ...
        f"- Answered questions: agent A {observed['agent_a_answered']}/{observed['questions']}; "
        f"agent B {observed['agent_b_answered']}/{observed['questions']}; jointly answered {observed['joint_answered']}.\n"
        f"- Agent-authored valid reports: A {observed['agent_a_authored_reports']}/{observed['tasks']}; "
