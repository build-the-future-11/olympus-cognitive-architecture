...
**Status: executable reference architecture, 2026-09-06.** Corresponding typed,
deterministic, and trainable component implementations now live under
`olympus/models/`. Neither these documents nor their unit-gradient tests are
evidence that a named family model has been trained, evaluated, or promoted.

The six names are roles on one governed substrate, not six independent
foundation-model pretraining projects:

The table below is the **target architecture and first decision gate**, not a
claim that every boundary is already implemented. The exact executable subset
and its process-local limitations are maintained in
...
| Aion | governed research orchestration | optional proposal/routing policy | protocol state machine, Pantheon audit, human approval policy | beats deterministic orchestration at equal budget and false-promotion rate |
...
The exact code/class map and focused verification commands are in
[`docs/model-families.md`](../../docs/model-families.md). The paper source and
the claim/source audit live in `research/paper/`.
